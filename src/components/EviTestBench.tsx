...
const gridDemoThemes: string[] = [
  ...
  "light", "dark", "secondary",
  // E. EviCard varianter
  "light", "primary-soft", "dark", "secondary-soft",
  // F. Nesting
  "primary", "light", "dark",
  // G. Split på soft/tint temaer
  "primary-soft", "secondary-soft", "primary-tint", "secondary-tint",
  // H. Full composition
  "light",
];
const gridDemoSlices: FakeSlice[] = gridDemoThemes.map((t) => ({ primary: { theme: t } }));
  ...
  return (
    <>
      ...
      {(() => {
        ...
        const softTintThemes = ["primary-soft", "secondary-soft", "primary-tint", "secondary-tint"] as const;

        return (
          <>
            {/* ── A. EviSplit — alle 5 presets ── */}
            {presets.map((preset, i) => (
              <EviSection key={`split-${preset}`} theme={g[i].theme} collapsePadding={g[i].collapsePadding}>
                ...
                <EviSplit preset={preset}>
                  <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <p className="text-sm font-mono opacity-60 mb-2">Venstre ({preset.split("-")[0]}%)</p>
                    <p>
                  ...
                  <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <p className="text-sm font-mono opacity-60 mb-2">Højre ({preset.split("-")[1]}%)</p>
            ...
            {/* ── B. EviSplit — 4 align varianter ── */}
            {aligns.map((align, i) => (
              <EviSection key={`align-${align}`} theme={g[5 + i].theme} collapsePadding={g[5 + i].collapsePadding}>
                <p className="col-span-12 text-sm font-mono opacity-50">
                ...
                <EviSplit preset="60-40" align={align}>
                  <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <p className="text-sm font-mono opacity-60 mb-2">Venstre — lang tekst</p>
                  ...
                  <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <p className="text-sm font-mono opacity-60 mb-2">Højre — kort</p>
                    <p>Kort indhold.</p>
                  </div>
                </EviSplit>
              </EviSection>
            ...
            {/* ── C. Realistisk slice ── */}
            <EviSection theme={g[9].theme} collapsePadding={g[9].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                C. EviSplit 50-50 — prose venstre, knapper højre (realistisk slice)
              </p>
              <EviSplit preset="50-50" align="center">
                <div className="evi-prose">
                  <h2>Vores mission</h2>
                  <p>Vi bygger værktøjer der gør det nemt for små virksomheder at se professionelle ud online. Ingen kode, ingen besvær.</p>
                </div>
                <div className="flex flex-col gap-4">
                  <EviButton variant="primary" appearance="solid" size="lg">Kom i gang</EviButton>
                  <EviButton variant="secondary" appearance="outline" size="lg">Læs mere</EviButton>
                </div>
              </EviSplit>
            </EviSection>

            {/* ── D. EviAutoGrid — alle 3 sizes ── */}
            <EviSection theme={g[10].theme} collapsePadding={g[10].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                D. EviAutoGrid size=&quot;sm&quot; — 8 items (logos/tags)
              </p>
              <EviAutoGrid size="sm">
                {Array.from({ length: 8 }, (_, i) => (
                  <div key={i} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4 text-center">
                    <div className="h-12 w-12 mx-auto rounded bg-[color-mix(in_oklch,currentColor_12%,transparent)]" />
                    <p className="text-xs font-mono opacity-60 mt-2">Logo {i + 1}</p>
                  </div>
                ))}
              </EviAutoGrid>
            </EviSection>

            <EviSection theme={g[11].theme} collapsePadding={g[11].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                D. EviAutoGrid size=&quot;md&quot; — 7 items (standard cards, wrapping test)
              </p>
              <EviAutoGrid size="md">
                {Array.from({ length: 7 }, (_, i) => (
                  <div key={i} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <div className="h-32 rounded bg-[color-mix(in_oklch,currentColor_12%,transparent)] mb-3" />
                    <p className="font-semibold">Feature {i + 1}</p>
                    <p className="text-sm opacity-70 mt-1">Kort beskrivelse af denne feature.</p>
                  </div>
                ))}
              </EviAutoGrid>
            </EviSection>

            <EviSection theme={g[12].theme} collapsePadding={g[12].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                D. EviAutoGrid size=&quot;lg&quot; — 3 items (pricing cards)
              </p>
              <EviAutoGrid size="lg">
                {["Basis", "Pro", "Enterprise"].map((name, i) => (
                  <EviCard key={i} rows={4} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-8 text-center">
                    <p className="text-xs font-mono opacity-50 uppercase tracking-wider">{name}</p>
                    <p className="text-4xl font-bold mt-2">{(i + 1) * 99} kr</p>
                    <p className="text-sm opacity-70 mt-1">per måned</p>
                    <div className="mt-6">
                      <EviButton variant="primary" appearance={i === 1 ? "solid" : "outline"} size="md">
                        Vælg {name}
                      </EviButton>
                    </div>
                  </EviCard>
            ...
            <EviSection theme={g[13].theme} collapsePadding={g[13].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
              ...
              <EviAutoGrid size="md">
                {[
                  ...
                  { title: "En meget længere titel der fylder flere linjer for at teste subgrid row alignment", desc: "Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur." },
                  { title: "Medium titel her", desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit." },
                  { title: "Kort", desc: "Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris." },
                  { title: "Femte kort — wrapping", desc: "Dette kort wrapper til en ny række." },
                ].map((item, i) => (
                  <EviCard key={i} rows={4} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4">
                    ...
                    <div className="mt-3">
                      <EviButton variant="primary" appearance="solid" size="sm">Læs mere</EviButton>
                    </div>
                  </EviCard>
                ))}
              </EviAutoGrid>
            ...
            <EviSection theme={g[14].theme} collapsePadding={g[14].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                E. EviCard rows=3 — kun titel + beskrivelse + CTA
              </p>
              <EviAutoGrid size="md">
                {[
                  { title: "Webdesign", desc: "Vi designer responsive websites der ser professionelle ud på alle enheder." },
                  { title: "SEO & Synlighed", desc: "Kort." },
                  { title: "Hosting & Support", desc: "Vi håndterer alt det tekniske så du kan fokusere på din forretning. 24/7 support inkluderet." },
                  { title: "E-mail Marketing Automation", desc: "Automatisér dine kampagner og nå dine kunder på det rigtige tidspunkt med det rigtige budskab." },
                ].map((item, i) => (
                  <EviCard key={i} rows={3} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <h3 className="text-lg font-semibold">{item.title}</h3>
                    <p className="text-sm opacity-70 mt-1">{item.desc}</p>
                    <div className="mt-4">
                      <EviButton variant="primary" appearance="text" size="sm" arrow>Læs mere</EviButton>
                    </div>
                  </EviCard>
                ))}
              </EviAutoGrid>
            ...
            <EviSection theme={g[15].theme} collapsePadding={g[15].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
              ...
              <EviAutoGrid size="sm">
                {[
                  ...
                  { title: "Sikkerhed", desc: "SSL, firewall og daglige backups er standard." },
                  { title: "Skalerbarhed", desc: "Fra 100 til 100.000 besøgende." },
                  { title: "Oppetid", desc: "99.9% garanteret oppetid med automatisk failover og redundans i flere datacentre." },
                  { title: "API", desc: "RESTful API til alle integrationer." },
                  { title: "Analytics", desc: "Real-time data." },
                ].map((item, i) => (
                  <EviCard key={i} rows={2} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4">
            ...
            <EviSection theme={g[16].theme} collapsePadding={g[16].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                E. EviCard rows=4 — tomt beskrivelse-felt (empty wrapper keeps alignment)
              </p>
              <EviAutoGrid size="lg">
                {[
                  { title: "Med beskrivelse", desc: "Denne har en beskrivelse som fylder plads." },
                  { title: "Uden beskrivelse", desc: "" },
                  { title: "Også med beskrivelse", desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit seddo." },
                ].map((item, i) => (
                  <EviCard key={i} rows={4} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <div className="h-20 rounded bg-[color-mix(in_oklch,currentColor_12%,transparent)]" />
                    <h3 className="text-lg font-semibold mt-3">{item.title}</h3>
                    <div>{item.desc && <p className="text-sm opacity-70 mt-1">{item.desc}</p>}</div>
                    <div className="mt-3">
                      <EviButton variant="secondary" appearance="solid" size="sm">Handling</EviButton>
                    </div>
                  </EviCard>
                ))}
              </EviAutoGrid>
            ...
            <EviSection theme={g[17].theme} collapsePadding={g[17].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                F. Nested: EviSplit 67-33 → EviAutoGrid size=&quot;sm&quot; i 67%-pane
              </p>
              <EviSplit preset="67-33">
                <div>
                  <p className="text-sm font-mono opacity-60 mb-4">67% pane — AutoGrid reagerer på sin egen bredde</p>
                  <EviAutoGrid size="sm">
                    {Array.from({ length: 6 }, (_, i) => (
                      <div key={i} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4 text-center">
                        <p className="text-sm font-mono opacity-60">Item {i + 1}</p>
                      </div>
                    ))}
                  </EviAutoGrid>
                </div>
                <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                  <p className="text-sm font-mono opacity-60 mb-2">33% sidebar</p>
                  <p>AutoGrid i venstre pane reagerer på sin egen bredde, ikke sektionens.</p>
                </div>
              </EviSplit>
            </EviSection>

            <EviSection theme={g[18].theme} collapsePadding={g[18].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                F. Nested: EviSplit 33-67 → EviAutoGrid size=&quot;md&quot; + EviCard i 67%-pane
              </p>
                ...
                <div>
                  <EviAutoGrid size="md">
                    {[
                      ...
                      { title: "Kort B med længere titel", desc: "Mere tekst her." },
                      { title: "Kort C", desc: "Endnu en beskrivelse." },
                      { title: "Kort D", desc: "Sidste kort i 67%-pane." },
                    ].map((item, i) => (
                      <EviCard key={i} rows={3} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4">
                        <h4 className="font-semibold">{item.title}</h4>
                        <p className="text-sm opacity-70 mt-1">{item.desc}</p>
                        <div className="mt-2">
                          <EviButton variant="primary" appearance="text" size="sm" arrow>Se mere</EviButton>
                        </div>
                      </EviCard>
                    ))}
                  </EviAutoGrid>
            ...
            <EviSection theme={g[19].theme} collapsePadding={g[19].collapsePadding}>
              <p className="col-span-12 text-sm font-mono opacity-50">
                F. Stress test: EviSplit 67-33 → EviAutoGrid size=&quot;md&quot; i 33%-pane (smal!)
              </p>
              <EviSplit preset="67-33">
                <div className="evi-prose">
                  <h2>Hovedindhold</h2>
                  <p>Den brede pane har prose-tekst. Den smalle 33%-pane til højre har et AutoGrid, der burde falde til 1 kolonne fordi der kun er ~400px tilgængeligt.</p>
                </div>
                <div>
                  <EviAutoGrid size="md">
                    {Array.from({ length: 3 }, (_, i) => (
                      <EviCard key={i} rows={2} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4">
                        ...
                        <p className="text-sm opacity-70 mt-1">Skal stå i 1 kolonne.</p>
                      </EviCard>
                    ))}
                  </EviAutoGrid>
                </div>
            ...
            {softTintThemes.map((theme, i) => (
              <EviSection key={`split-theme-${theme}`} theme={g[20 + i].theme} collapsePadding={g[20 + i].collapsePadding}>
                <p className="col-span-12 text-sm font-mono opacity-50">
                ...
                <EviSplit preset="60-40" align="center">
                  <div className="evi-prose">
                    ...
                    <p>Tekst og knapper skal have korrekt kontrast uanset baggrundstema.</p>
                    <EviButton variant="primary" appearance="solid" size="md">Primær solid</EviButton>
                  </div>
                  <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                    <p className="text-sm opacity-70">Højre kolonne med surface-neutral baggrund.</p>
                    <div className="mt-3">
                      <EviButton variant="secondary" appearance="outline" size="sm">Sekundær outline</EviButton>
                    </div>
                  </div>
                </EviSplit>
              </EviSection>
            ...
            <EviSection theme={g[24].theme} collapsePadding={g[24].collapsePadding}>
              <div className="col-span-12 evi-prose max-w-prose">
                <h2>H. Full composition — flere blokke i én sektion</h2>
                <p>Denne sektion demonstrerer gap-y mellem stakkede children: først prose, så et split, og til sidst et card-grid. Alt med automatisk vertikal afstand via EviSections gap-y.</p>
              </div>
              <EviSplit preset="50-50" align="center">
                <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
                  <p className="font-semibold mb-2">Venstre blok</p>
                  <p className="text-sm opacity-70">Denne blok er en del af et 50-50 split midt i sektionen.</p>
                </div>
                <div className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-6">
              ...
              <EviAutoGrid size="md">
                {Array.from({ length: 4 }, (_, i) => (
                  <EviCard key={i} rows={3} className="rounded-lg bg-[color-mix(in_oklch,currentColor_8%,transparent)] p-4">
                    ...
                    <p className="text-sm opacity-70 mt-1">Under splittet — samme sektion, automatisk gap-y.</p>
                    <div className="mt-2">
                      <EviButton variant="neutral" appearance="outline" size="sm">Handling</EviButton>
                    </div>
                  </EviCard>
                ))}
              </EviAutoGrid>
