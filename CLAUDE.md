...
- **Check first**: grep/read/fetch FØR du skriver. **Før du bygger NOGET nyt (komponent/layout/helper): glob `src/components/{layout,ui,typography}` + `src/lib/` og gennemgå ALLE Evi-komponenter/helpers** — genbrug eller udvid frem for at lave nyt. (`feedback_check_first_reuse_always`)
  ...
  - **Rå layout-`<div>` = "noget nyt" → slå op i tabellen FØR du skriver klassen** (`feedback_no_raw_layout_divs`). Trigger: skal du skrive `<div>` med `flex`/`grid`/`gap-`/`grid-cols`/`col-span`/`subgrid`/`space-y` → STOP, det HAR en Evi-komponent. `flex flex-col gap` → **EviStack** · `flex gap` (row) → **EviStack** `direction="row"` · `grid grid-cols-N` → **EviAutoGrid** · 2-panel → **EviSplit** · `col-span-12` → **EviRow** · forskudt/masonry → **EviMasonry** · kort → **EviCard**. Mangler prop → udvid med ÉN. Rå div KUN når ingen komponent kan udtrykke kravet (fx responsivt gap, container-query kolonne-antal) + WHY-kommentar der navngiver komponenten den erstatter (skabelon: `TextWithImagesDuoLayout.tsx`s imageEl). **Opfind aldrig nye gap-tiers/breakpoints** (`tight`, `@[500px]` blev afvist) — genbrug systemets `md:`/`lg:` + AutoGrids `@[…]px`. `data-slot` = engelsk kebab-case.
